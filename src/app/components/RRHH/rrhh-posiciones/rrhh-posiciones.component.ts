...
@Component({
  selector: 'app-rrhh-posiciones',
  templateUrl: './rrhh-posiciones.component.html',
  styleUrls: ['./rrhh-posiciones.component.css']
})
export class RrhhPosicionesComponent {
  loading = false;
  edit: any = {};
  // table
  title = 'Posiciones';
  dataR: any;
  url_table = `${environment.api_url}posiciones`;
  columnsTable: any = [
    { name: 'Acc.', field: 'actions' },
    { name: 'Nombre', field: 'posicion' },
  ...
  delete() {
    this.http
      .delete(`${environment.api_url}posiciones/${this.edit?.id}`)
      .subscribe(
        (response: any) => {
          this.loading = false;
