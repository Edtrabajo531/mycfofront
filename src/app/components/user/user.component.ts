...
export class UserComponent {
  ...
  title = 'Usuarios';
  dataR: any;
  url_table = `${environment.api_url}usuarios`;
  columnsTable: any = [
    { name: 'Acc.', field: 'actions', type: '' },
    { name: 'Nombre', field: 'name', type: '' },
    { name: 'Correo', field: 'email', type: '' },

    { name: 'Roles', field: 'roles', type: 'array' },
  ...
  openModal(modalcreate: any, edit: any = false, size: any = false) {
    if (edit) {
      this.edit = edit;
      if (this.edit.roles[0]) {
        this.edit.role_id = this.edit.roles[0].id;
      }
    } else {
      this.edit = {};
    }
