...
export class ProductosComponent {
  filtros_a_usar = [];
  // filtros_a_usar:any = [{name:"empresa_id",value:''}];
  url_datos_filtros = `${environment.api_url}productos-recursos`;
  loading = false;
  edit: any = {};
  loaded = false;
  // table
  ...
  columnsTable: any = [
    ...
    { name: 'Precio venta', field: 'precio_venta' },
    { name: 'Costo unitario', field: 'costo_unitario' },
    { name: 'Empresa', field: 'empresa' },
    { name: 'Unidad venta', field: 'unidad_venta' },
    { name: '%', field: 'contribucion_producto' },
    { name: 'Contribución', field: 'utilidad_producto' },
    { name: 'Creado', field: 'created_at',type:'date' },
  ];
  actionsTable: any = [
    {
      numbre: '',
  ...
  getEmpresa(){
    ...
    this.url_table = `${environment.api_url}productos?empresa_id=${this.empresa_id}`;
    this.loaded = true;
  }
  ...
  delete() {
    this.http
      .delete(`${environment.api_url}productos/${this.edit?.id}`)
      .subscribe(
        (response: any) => {
          this.loading = false;
