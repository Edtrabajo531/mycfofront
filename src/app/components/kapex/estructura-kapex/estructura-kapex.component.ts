...
@Component({
  selector: 'app-estructura-kapex',
  templateUrl: './estructura-kapex.component.html',
  styleUrls: ['./estructura-kapex.component.css'],
})

export class EstructuraKapexComponent {
  loading = false;
  edit: any = {};
  // table
  title = 'Estructura capex';
  nameDataResponse = 'capexes';
  dataR: any;
  url_table = '';
  empresa_id:any = '';
  loaded = false;
  ...
  getEmpresa() {
    ...
    this.url_table = `${environment.api_url}capex?empresa_id=${this.empresa_id}`;
    this.loaded = true;
  }
  ...
  delete() {
    this.http.delete(`${environment.api_url}capex/${this.edit?.id}`).subscribe(
      (response: any) => {
